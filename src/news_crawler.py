...
def crawl_url(search_keyword, start_date, end_date):
    ...
    error_list = []  # 에러 로그 리스트
    temp_files_folder = f"./url/temp_files"
    util.create_folder(temp_files_folder)  # 폴더 만들기 : temp_crawling_result
    ...
    if not util.is_folder_empty(temp_files_folder):
        print("Hi")
        temp_date = util.find_latest_date(temp_files_folder)    # YYYYMMDD 형식
        latest_date = f"{temp_date[:4]}-{temp_date[4:6]}-{temp_date[6:]}"
        start_date = util.get_next_date(latest_date)
        print(start_date)
    ...
    for date in date_list:
        ...
        # [1-3. 하루 단위로 임시파일 생성]
        cr.make_url_temp_file(row_list, date_no_dots, search_keyword)

    # [2. 크롤링 결과 파일 만들기 : .csv 파일]
